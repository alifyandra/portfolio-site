...
Same events on both hops; the backend relays them. One JSON object per line. One
exception: `provisioning` is generated by the backend (not the sidecar) during
Fargate cold start and is sent to the browser only (see the 2026-07-07 amendment in
ADR 11).

| `type`     | Fields | Meaning |
|------------|--------|---------|
| `provisioning` | `message` (string) | Cold-start status while the backend launches the Fargate sidecar task (fargate mode only, emitted zero or more times before the first `qr`). Backend-originated, browser only, no batch effect. In static mode it never appears. |
...
| `provisioning` | none (backend-generated during Fargate cold start; browser only) |
