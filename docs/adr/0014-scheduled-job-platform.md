...
finance-scraper server later (issue #74).
...
- **Artifact** is a stored piece of job I/O: for the digest, one per Source. It is
  ...
  runner, whose scraped data lands here for a claimer to pick up.
...
`digest.llm`. Registering a new workload later (the finance scraper) is "insert a
...
- **On an external runner that pulls the work.** Alif's laptop, where Claude Code is
  signed in on the subscription, today; the home finance-scraper server (issue #74)
...
- Two ways to run an LLM step coexist on purpose: unattended on the box, or
  ...
  is the seam the finance scraper reuses without new architecture.
